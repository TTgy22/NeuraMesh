...
- **收益实时化 + 任务模拟计算（2026-06-12 晚）**：①NodeService 收益采样器（5s 对已注册节点记 (HH:mm:ss,totalEarned) 环形 120 点），earnings() 优先返回采样序列（阶梯上扬曲线），无采样回退按天均摊；②ResourceGroupService.allocateTask 5 参重载 simulateMs（HTTP 默认 6000，service 4 参=0 即时保旧语义）：RUNNING 入 groupTasks → simulator 调度器到点 settleOnChain 真实上链 → SETTLED；`GET /groups/tasks/{id}` 轮询；③前端：VendorConsole RUNNING 轮询 1s+滑动进度条"节点计算中…"，NodeDashboard 5s 轮询 status，EarningsChart props +refreshKey=totalEarned；④测试 GroupTaskSimulationTest（RUNNING→SETTLED→executed + 即时路径）。api 12 测试/node 6/dashboard 1 全绿。
- **历史任务后端权威化（2026-06-12）**：VendorConsole history 原为组件 state 切页即丢——`allGroupTasks()`（groupTasks 注册表按 gtask 序号倒序）+ `GET /groups/tasks`；前端挂载拉取全量并对 RUNNING 项恢复轮询，切页/刷新均不丢。GroupTaskSimulationTest +注册表断言。
- **品牌 logo（2026-06-12）**：AI 生成神经网格风格图标（紫青渐变节点连线，源图 .cursor 项目 assets），512px 方形部署于两前端 `public/logo.png`（favicon + 侧栏/header img）、Electron 窗口/托盘图标（main.ts appLogo()：dist/logo.png 缺失回退空图标）、launcher.ps1 窗体 Icon（Bitmap.GetHicon 运行时转换）。exe 文件图标未换（需 rcedit，赛后再说）。
- **【vite dev 长会话坑】**隔夜 dev server 页面跑旧 bundle 且同 URL hash 导航不触发重载——强制刷新需变更 query（?fresh=1#/...）；浏览器演示前先确认页面代码是否最新。
- **图形化启动器（任意 Windows 免环境）**：①`jlink` 精简 JRE（79MB，模块集 java.base/logging/naming/desktop/management/security.jgss/sasl/sql/xml/instrument/net.http/jdk.unsupported/crypto.ec/crypto.cryptoki/zipfs，实测带起 Spring Boot+Netty+RocksDB）打进 release\jre\；②`release\launcher\launcher.ps1` WinForms 控制面板（三服务状态灯 3s 轮询/启停/一键全启/日志框/关窗询问停服；**ps1 必须 UTF-8 BOM** 否则 PowerShell 5.1 中文乱码；入口 `NeuraMesh启动器.bat` 用 -ExecutionPolicy Bypass -STA -WindowStyle Hidden）；③`serve-dashboard.ps1` HttpListener 静态托管 dashboard 于 5173（MIME 表+防穿越+SPA fallback，实测 js/css MIME 正确）；④backendLauncher.ts/launcher.ps1 找 java 顺序：包内 jre\bin\java.exe → 系统 PATH；⑤一键全启自动开浏览器 http://localhost:5173（前端入口）。最终 zip 约 224MB。
